...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_student_id"])) {
    // Get the student ID to be deleted from the form data and sanitize it to prevent SQL injection
    $deleteStudentId = mysqli_real_escape_string($link, $_POST["delete_student_id"]);

    // Prepare and execute the query to delete the student from the database
    $deleteQuery = "DELETE FROM pupil WHERE pupil_id = '$deleteStudentId'";
    if (mysqli_query($link, $deleteQuery)) {
        echo "<p>Student deleted successfully.</p>";
    } else {
        echo "<p>Error deleting student: " . mysqli_error($link) . "</p>";
    }
}
...
<div class="content">
    ...
    <!-- Display student data in an HTML table with delete buttons -->
        ...
        <table id="studentTable">
            ...
            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                <tr>
                    ...
                    <td><?php echo $row['guardian1_id']; ?></td>
                    <td><?php echo $row['class_id']; ?></td>
                    <td>
                        <!-- Add a unique ID to each form and hidden field dynamically -->
                        <form class="delete-form" method="post" action="">
                            <input type="hidden" name="delete_student_id" value="<?php echo $row['pupil_id']; ?>">
